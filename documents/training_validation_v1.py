...
Date: 4/Oct/2019
author: CMendezC
Modificated: The Fantastic Four
''' 
...
##    |
##    v
## python training-validation-v3.py  --inputPath /export/space1/users/mpantoja/3_Sem/BioInf/CRF_Tarea/data-sets/ --trainingFile training-data-set-70.txt --testFile test-data-set-30.txt --outputPath ./
...
def word2features(sent, i):
    ...
    features = {
        ...
        #'lemma': lemma,
        #'postag': postag,
##Intercalo de orden para aumentar el score de lemas
    'lemma[-3:]': lemma[-4:],
        'lemma[-2:]': lemma[-2:],
        'lemma[-1:]': lemma[-1:],
    'lemma[+3:]': lemma[:5],
    ...
    with open(os.path.join(options.inputPath, options.trainingFile), "r") as iFile:
        for line in iFile.readlines():
            ...
            for token in line.split():
                ...
                ##print('-------------------------------- QueSoyInicio listToken1--------------------------------')
                ##print(token)

                if options.excludeStopWords:##NoEntra
                    listToken = token.split('|')

                    ##print('-------------------------------- QueSoyInicio listToken2 --------------------------------')
                    ##print(listToken)

                    lemma = listToken[1]
                    if lemma in stopwords:
                        continue
                if options.excludeSymbols:##NoEntra
                    listToken = token.split('|')
                    lemma = listToken[1]

                    ##print('-------------------------------- QueSoyInicio lemma --------------------------------')
                    ##print(lemma)
...
##    print('-------------------------------- QueSoyFin --------------------------------')
        ...
        for line in iFile.readlines():
            ...
            for token in line.split():
                if options.excludeStopWords:
                    ##print("Entre a tu cora 1")
                    listToken = token.split('|')
                    lemma = listToken[1]
                    if lemma in stopwords:##NoEntra
                        ##print("Entre a tu cora 1.1")
                        continue
                if options.excludeSymbols:
                    ##print("Entre a tu cora 2")
                    listToken = token.split('|')
                    lemma = listToken[1]
                    if lemma in symbols:##NoEntra
                        ##print("Entre a tu cora 2.1")
                        continue
                listLine.append(token)
                ##print('-------------------------------- QueSoyInicio Token--------------------------------')
                ##print(token)
    ...
    ##print('-------------------------------- QueSoyInicio Callados--------------------------------')
    #print(sent2features(sentencesTrainingData[0])[0])##Callar
    #print(sent2features(sentencesTestData[0])[0])##Callar
    t0 = time()

    X_train = [sent2features(s) for s in sentencesTrainingData]

    ##print('-------------------------------- QueSoyInicio X_train--------------------------------')
    ##print(X_train)

    y_train = [sent2labels(s) for s in sentencesTrainingData]

    ##print('-------------------------------- QueSoyInicio y_train--------------------------------')
    ##print(y_train)

    X_test = [sent2features(s) for s in sentencesTestData]
    ##print('-------------------------------- QueSoyInicio X_test--------------------------------')
    ##print(X_test)

    y_test = [sent2labels(s) for s in sentencesTestData]
    ##print('-------------------------------- QueSoyInicio y_test--------------------------------')
    ##print(y_test)
    ...
    ##Se incremento las iteraciones maximas para un mejor entrenamiento
    crf = sklearn_crfsuite.CRF(
        algorithm='lbfgs',
        max_iterations=300,
        all_possible_transitions=True
